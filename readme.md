...
### Changelog
- **2025-08-10**: Added `./util/client_transcribe_advanced.py` file and updated README.
...
对于超长时长的音视频文件（例如超过3小时），标准的转录功能可能会遇到失败或卡住的问题。

`client_transcribe_advanced.py` 脚本提供了以下高级功能来解决此问题：

1.  **临时提取音频**：从视频文件中提取音频轨道。
2.  **自动分割音频**：将长音频文件分割成多个较短的片段。
3.  **逐一转录**：对每个音频片段独立进行语音转文字转录。
4.  **智能合并**：将所有片段的转录结果（SRT字幕文件）合并成一个最终的、时间戳准确的SRT文件。

此功能有效地突破了原始转录功能的时长限制，并确保了合并后字幕时间戳的准确性，特别适合处理长时间录音、会议录像、课程视频等。

## 功能：客户端自动服务管理及自启动

`core_client.py` 现在包含以下增强功能，旨在提升用户体验和简化部署：

1.  **自动拉起服务端**：
    *   当 `core_client.py` 启动时，它会自动检查 `core_server.py` 是否正在运行。
    *   如果服务端未运行，客户端将自动尝试启动 `core_server.py`，确保用户无需手动操作即可开始使用语音输入功能。
    *   此设计是单向的：`core_server.py` 启动后不会尝试拉起 `core_client.py`，保持了清晰的逻辑层次。

2.  **开机自启动配置**：
    *   客户端支持配置为开机自启动，方便用户在系统启动后立即使用 CapsWriter-Offline。
    *   相关的配置选项已集成到 `config.py` 文件中，`boot_auto_start = True`，默认开启此功能。用户可以根据需要进入 `config.py` 文件进行修改。
    *   此开机自启动功能已在 **Windows 11** 系统下通过修改注册表的方式完成测试，确保其可靠性。
...
1. 运行 `core_server.py` 脚本，服务器会首先快速加载核心的 Paraformer 语音识别模型，并立即开始监听客户端连接。标点符号模型将在后台异步加载，不会阻塞服务启动。
    *   **服务器启动时间**：现在仅需几秒钟（语音模型加载时间），无需等待标点模型加载完成。
    *   **初期使用**：在标点模型加载完成前，语音转写功能已可用，但返回结果将暂时不包含标点符号。加载完成后，服务会自动恢复完整的标点恢复功能。
    *   **内存占用**：模型全部加载完毕后，总计占用约 2GB 内存。
